...
def create_app(test_config=None):
    ...
    def collect_candidates():
        ...
        if request.method == "POST":
            ...
            try:
                ...
                stats = {"source": source_label, "fetched": len(addresses), "inserted": 0, "skipped": 0}
                with db() as con:
                    for address in addresses:
                        ...
                        con.execute(
                            """INSERT INTO lead_candidates
                               (prefecture, city, address, source_url, source_type, normalized_key)
                               VALUES (?, ?, ?, ?, ?, ?)""",
                            ("宮城県", city, address, actual_source_url, source_type, key),
                        )
                        stats["inserted"] += 1
                flash(f"{source_label}: {stats['inserted']}件を追加、{stats['skipped']}件をスキップしました。", "success")
            except (requests.RequestException, ValueError, OSError) as exc:
                flash(str(exc), "error")
        return render_template("collect.html", stats=stats)
    ...
    def candidates():
        ...
        with db() as con:
            rows = con.execute(
                "SELECT * FROM lead_candidates WHERE status=? ORDER BY updated_at DESC, id DESC",
                (status,),
            ).fetchall()
    ...
    def candidate_detail(candidate_id):
        ...
        priority, reason = candidate_priority(candidate)
        return render_template("candidate_detail.html", candidate=candidate, priority=priority, priority_reason=reason,
                               research_statuses=RESEARCH_STATUSES)

    @app.get("/export.csv")
    def export_csv():
        with db() as con:
            rows = con.execute("SELECT * FROM facilities ORDER BY updated_at DESC").fetchall()
        output = io.StringIO()
        writer = csv.writer(output)
        writer.writerow(["name", "company_name", "prefecture", "city", "address", "facility_type", "official_url", "phone", "email", "contact_url", "priority", "status", "notes"])
        for row in rows:
            writer.writerow([row[key] for key in ["name", "company_name", "prefecture", "city", "address", "facility_type", "official_url", "phone", "email", "contact_url", "priority", "status", "notes"]])
        return Response("\ufeff" + output.getvalue(), mimetype="text/csv", headers={"Content-Disposition": "attachment; filename=facilities.csv"})
