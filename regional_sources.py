...
SOURCES = {
    "aomori": {"prefecture": "青森県", "title": "住宅宿泊事業法届出住宅一覧", "url": "https://opendata.pref.aomori.lg.jp/dataset/2047.html", "format": "XLSX（オープンデータ）", "mode": "import", "note": "県公式オープンデータのXLSXをヘッダー名で判定し、届出住宅所在地を候補へ取り込み。"},
    "iwate": {"prefecture": "岩手県", "title": "民泊（住宅宿泊事業）について", "url": "https://www.pref.iwate.jp/kurashikankyou/anzenanshin/seikatsueisei/1016554/1016555.html", "format": "PDF（届出状況一覧）", "mode": "source_only", "note": "公式PDFは届出状況の集計中心。個別住所を誤抽出しないため自動登録は停止。"},
    "akita": {"prefecture": "秋田県", "title": "住宅宿泊事業者一覧", "url": "https://www.pref.akita.lg.jp/pages/archive/31592", "format": "PDF（届出者・住宅所在地・電話）", "mode": "import", "note": "県公式PDFの列位置を確認し、届出者住所と届出住宅所在地を区別して候補へ取り込み。"},
    "yamagata": {"prefecture": "山形県", "title": "住宅宿泊事業者一覧", "url": "https://www.pref.yamagata.jp/020071/kenfuku/doubutsuaigo/eisei/seikatsueisei/minpaku/minpaku.html", "format": "PDF（届出住宅所在地）", "mode": "import", "note": "県公式PDFが届出住宅所在地を一覧公開しているため、安全に住所候補として取り込み。"},
    "fukushima": {"prefecture": "福島県", "title": "住宅宿泊事業法の受理済み届出住宅一覧", "url": "https://www.pref.fukushima.lg.jp/sec/32031a/minpaku-04.html", "format": "HTML（商号・屋号・住所等）", "mode": "import", "note": "県公式HTMLの表から商号・屋号・届出住宅住所・公開電話を取り込み。"},
}
...
def _get(url, accept="text/html,*/*"):
    response = requests.get(url, timeout=20, headers={"User-Agent": "Mozilla/5.0 TohokuLodgingSalesAI/1.0", "Accept": accept, "Accept-Language": "ja,en;q=0.7"}, allow_redirects=True)
...
def _discover_resource(page_html, base_url, suffix, label_keyword=""):
    ...
    for link in soup.find_all("a", href=True):
        ...
        if suffix.lower() not in href.lower():
            continue
        score = 1 + (4 if label_keyword and label_keyword in label else 0) + (2 if label_keyword and label_keyword in href else 0)
...
def _header_index(values, aliases):
    ...
    normalized_aliases = [_normalize(alias).replace("・", "").replace("、", "") for alias in aliases]
...
def parse_aomori_xlsx(xlsx_bytes):
    ...
    for sheet in workbook.worksheets:
        ...
        for row in rows[header_row + 1:]:
            ...
            records.append({"address": address, "name": name, "company_name": operator if any(word in operator for word in CORPORATE_WORDS) else "", "phone": phone, "official_url": ""})
    if not records:
...
def discover_akita_pdf(page_html, base_url):
    ...
    for link in soup.find_all("a", href=True):
        href = link.get("href", "").strip()
        label = link.get_text(" ", strip=True)
        if ".pdf" in href.lower() and ("住宅宿泊事業者一覧" in label or "住宅宿泊事業者一覧" in href):
...
def parse_akita_pdf(pdf_bytes):
    ...
    number_pos, owner_pos, owner_address_pos, residence_pos, phone_pos = [header_line.find(label) for label in ("届出番号", "届出者氏名", "届出者住所", "届出住宅の所在地", "電話番号")]
    if min(number_pos, owner_pos, owner_address_pos, residence_pos, phone_pos) < 0:
        raise ValueError("秋田県PDFの列位置を確認できませんでした。")
    starts = [index for index, line in enumerate(lines) if re.match(r"^\s*\d+\s+M\d+", line)]
    records, seen = [], set()
    for pos, start in enumerate(starts):
        end = starts[pos + 1] if pos + 1 < len(starts) else len(lines)
        owner_parts, owner_address_parts, residence_parts, phone_parts = [], [], [], []
        ...
        owner_address = re.sub(r"\s+", " ", " ".join(part for part in owner_address_parts if part)).strip()
        residence = re.sub(r"\s+", " ", " ".join(part for part in residence_parts if part)).strip()
        ...
        if key in seen:
            continue
        seen.add(key)
        records.append({"address": residence, "name": "", "company_name": owner if any(word in owner for word in CORPORATE_WORDS) else "", "phone": phone, "official_url": ""})
    if not records:
...
def discover_yamagata_pdf(page_html, base_url):
    soup = BeautifulSoup(page_html, "html.parser")
    for link in soup.find_all("a", href=True):
        label, href = link.get_text(" ", strip=True), link.get("href", "")
...
def parse_yamagata_pdf(pdf_bytes):
    ...
    for raw in text.splitlines():
        ...
        if not line or "届出住宅" in line or "住宅宿泊" in line or "現在" in line or not re.match(r"^(.+?郡.+?[町村]|.+?[市町村])", line):
            continue
        key = _normalize(line)
        if key not in seen:
            seen.add(key)
...
def parse_fukushima_html(html):
    ...
    for row in soup.select("table tr"):
        ...
        if not address or not re.search(r"[市町村郡]", address):
            continue
        records.append({"address": address, "name": name, "company_name": company if any(word in company for word in CORPORATE_WORDS) else "", "phone": phone, "official_url": ""})
    return records
...
def import_records(database, source_key, records, source_url):
    source, prefecture = SOURCES[source_key], SOURCES[source_key]["prefecture"]
    ...
    with sqlite3.connect(database) as con:
        for record in records:
            ...
            con.execute("""INSERT INTO lead_candidates
                ...
                (record.get("name", ""), record.get("company_name", ""), prefecture, city, address, record.get("official_url", ""), record.get("phone", ""), source_url, f"{prefecture} {source['title']}", key, f"[{prefecture}公式資料から収集] {source_url}"))
            new_count += 1
...
def register_regional_sources(app):
    ...
    def import_regional_source():
        ...
        try:
            fetchers = {"aomori": fetch_aomori_records, "akita": fetch_akita_records, "yamagata": fetch_yamagata_records, "fukushima": fetch_fukushima_records}
            ...
            flash(f"{source['prefecture']}：公式資料{stats['total']}件を確認し、新規{stats['new']}件・重複{stats['duplicate']}件でした。", "success")
